...
my $main_png = "$foswiki_base/pub/Sandbox/$main_web_sample_topic/DirectedGraphPlugin_1.png";
...
my $topic_png = "$foswiki_base/pub/System/DirectedGraphWebMapPlugin/DirectedGraphPlugin_1.png";
...
copy($main_png, "$cwd/../../../../pub/System/DirectedGraphWebMapPlugin/SampleMainWebMap.png") or die $!;
...
$main_map_content =~ s/http:\/\/.*?\/view.*?\/Main/%SCRIPTURLPATH{"view"}%\/%MAINWEB%/sg or die;
$main_map_content =~ s/\n/ /g;

copy($topic_png, "$cwd/../../../../pub/System/DirectedGraphWebMapPlugin/sample.png") or die $!;
...
$topic_map_content =~ s/http:\/\/.*?\/view.*?\/System/%SCRIPTURLPATH{"view"}%\/%SYSTEMWEB%/sg or die;
$topic_map_content =~ s/\n/ /g;
...
$topic_text =~ s/(<map id="SampleMainWebMap" name="SampleMainWebMap">).*?(<\/map>)/$1$main_map_content$2/s or die;
$topic_text =~ s/(<map id="sample" name="sample">).*?(<\/map>)/$1$topic_map_content$2/s or die;
