...
sub _TOPICMAP {
	...
	unless (Foswiki::Func::topicExists($params{"web"}, $params{"topic"})) {
		return "\%RED\% !DirectedGraphWebMapPlugin error: Topic $params{web}.$params{topic} does not exist \%ENDCOLOR\%";
	}
    ...
    unless (defined $params{"backlinks"}) {
        if (defined $params{"links"}) {
			...
			$params{"backlinks"} = Foswiki::Func::getPluginPreferencesValue("BACKLINKS");
			unless (defined $params{"backlinks"}) {
				$params{"backlinks"} = Foswiki::Func::getPluginPreferencesValue("LINKS");
				unless (defined $params{"backlinks"}) {
					$params{"backlinks"} = 1;
				}
			}
    ...
    if ($debug) {
        &Foswiki::Func::writeDebug("$pluginName: \$params{$_} = ".(defined($params{$_})?"\"$params{$_}\"":"undef")) foreach (sort keys %params);
    }
    ...
    my $dot_options = '';
    $dot_options .= "file=\"".$params{"file"}."\" " if defined $params{"file"};
    $dot_options .= "engine=\"".$params{"engine"}."\" " if defined $params{"engine"};
	...
	push @returnlist, "node [height=\"0.3\", fontsize=14];"; # makes the text fill the box
    push @returnlist, "size=\"".$params{"size"}."\";";
    push @returnlist, "rankdir=$params{rankdir};";
    my $webbgcolor = Foswiki::Func::getPreferencesValue( "WEBBGCOLOR", $params{"web"} );
    ...
    push @returnlist, qq(node [style=filled, color="$webbgcolor", fontcolor="$fontcolor"];);

	# generate the "focus" node here,
	# and forwardlinks and backlinks only generate outer nodes.
	push @returnlist, qq("$params{topic}" [URL=").Foswiki::Func::getViewUrl($params{"web"},$params{topic}).qq("];);
	$webmap->{$params{"topic"}}{$nodeUrlOutput} = 1;
    # forward links
    push @returnlist, forwardlinks($params{"links"}, $params{"topic"}, $webmap, $params{'web'});
    # back links
    push @returnlist, backlinks($params{"backlinks"}, $params{"topic"}, $webmap, $params{'web'});
...
sub _WEBMAP {
    ...
    if ($debug) {
        &Foswiki::Func::writeDebug("$pluginName: \$params{$_} = ".(defined($params{$_}?"'$params{$_}'":
				'undef'))) foreach (sort keys %params);
    }
...
sub commonParameterDefaults
{
    ...
    unless (defined $params->{"expand"}) {
		$params->{"expand"} = Foswiki::Func::getPluginPreferencesValue("EXPAND");
        ...
        foreach my $targetTopic (sort keys %{$webmap->{$baseTopic}}) {
            ...
            if ($webmap->{$baseTopic}{$targetTopic}) {
				...
				if (not $webmap->{$targetTopic}{$nodeUrlOutput}) {
                	push @returnlist, qq("$targetTopic" [URL="$targetTopicUrl"];);
...
sub populateWebMapArray {
    ...
    my %webmap; # $webmap{$baseTopic}{$targetTopic} = 1 if $baseTopic links to $targetTopic.  DOES NOT CROSS WEBS
    ...
    $viewurl =~ s/^$urlhost//;
    $viewurl =~ s:/TOPIC$::;
    my $href = qr(<a[^>]+href\s*=\s*"(?:$urlhost)?$viewurl/(\w+)(?:#\w+)?"[^>]*>);
    ...
    foreach my $baseTopic (@topicList) {
		...
		{
			$debug && Foswiki::Func::writeDebug("${pluginName}: Skipping $web.$baseTopic (excluded)");
			next;
		}
        $debug && Foswiki::Func::writeDebug("${pluginName}: Scanning $web.$baseTopic");

        my $baseTopicText = Foswiki::Func::readTopicText($web, $baseTopic, "", 1);

        # expand WEB and TOPIC variables
        $baseTopicText =~ s/(%(?:HOME|NOTIFY|WEBPREFS|WIKIPREFS|WIKIUSERS)?TOPIC%)/Foswiki::Func::expandCommonVariables($1, $baseTopic, $web)/ge;
        $baseTopicText =~ s/%MAINWEB%/$Foswiki::cfg{UsersWebName}/ge; # faster than expandCommonVariables
...
#         $baseTopicText =~ s/\[\[([^\]]*)(\]\[)?([^\]]*)\]\]/$1/g;
        ...
        for my $var (@vars)
        {
          $baseTopicText =~ s/(%$var%)/Foswiki::Func::expandCommonVariables($1, $baseTopic, $web)/ge;
          $baseTopicText =~ s/(%$var\{.*?}%)/Foswiki::Func::expandCommonVariables($1, $baseTopic, $web)/ge;
        }

        # ... in fact, throw away ALL remaining variables
        $baseTopicText =~ s/%\w+[^%]*%//g; # The \w+ makes it more robust with respect to errors like $RED%. Note that [^%] matches a newline.
                                           # Discarding variables can also discard links if there are markup errors like the following:
                                           # %RED$ SomeLink %ENDCOLOR%

        my $renderedTopic = Foswiki::Func::renderText($baseTopicText, $web, $baseTopic);
        my @links = $renderedTopic =~ /$href/g;
        while (@links) {
            my $targetTopic = shift @links;
            if (exists $excludeTopic{$targetTopic})
			{
				$debug && Foswiki::Func::writeDebug("Skipping $baseTopic -> $targetTopic ($targetTopic excluded)");
				next;
			}
            $debug && Foswiki::Func::writeDebug("$baseTopic -> $targetTopic");
