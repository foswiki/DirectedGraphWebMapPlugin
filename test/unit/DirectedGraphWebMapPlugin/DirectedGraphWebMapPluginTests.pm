...
sub set_up {
	...
	{
        $this->writeTopic($this->{target_web}, $topic, $alternate_text{$topic});
    }
...
sub mapTest {
	...
	my $WEBBGCOLOR = Foswiki::Func::getPreferencesValue("WEBBGCOLOR", $options->{web});
	...
	{
		$this->assert(scalar($dot_attrs =~ s/^\s*engine="([^"]*)"\s*//o), "<dot> parameters: $dot_attrs");
		$this->assert_str_equals($1, $options->{engine}, $1);
	}
	if (exists $options->{file})
	{
		$this->assert(scalar($dot_attrs =~ s/^\s*file="([^"]*)"\s*//o), "<dot> parameters: $dot_attrs");
		$this->assert_str_equals($1, $options->{file}, $1);
	}
	$this->assert(scalar($dot_attrs =~ s/map=1\s*//o), "<dot> parameters: $dot_attrs");
	# There should only be (at most) whitespace left in the dot attributes
	$this->assert_does_not_match(qr/\S/, $dot_attrs, "<dot> parameters: $dot_attrs");
    ...
    $this->assert(scalar($output =~ s/^node\s*\[shape=(.+?)\];\s*//o), $output);
    $this->assert_str_equals($1, $options->{shape}, $1);
	
    $this->assert(scalar($output =~ s/^node\s*\[height="(.+?)", fontsize=(.+?)\];\s*//o), $output);
    ...
    $this->assert(scalar($output =~ s/^node \[style=filled, color="([^"]*)", fontcolor="([^"]*)"\];\s*//o), $output);
    $this->assert_str_equals($1, $WEBBGCOLOR);
	$this->assert($2 eq 'black' or $2 eq 'white');

	foreach my $topic (keys %{$options->{links}})
	{
		$this->assert(scalar($output =~ s/"$topic"\s*\[URL="([^"]*)"\];\s*//), "$topic: $output");
		$this->assert_str_equals($1, Foswiki::Func::getViewUrl($options->{web},$topic), $1);

		next unless defined $options->{links}->{$topic};

		foreach my $destination (@{$options->{links}->{$topic}})
		{
			$this->assert(scalar($output =~ s/"$topic"\s*->\s*"$destination";\s*//), "$topic->$destination: $output");
		}
	}
...
sub test_prefsWebMap {
	...
	Foswiki::Func::setPreferencesValue('DIRECTEDGRAPHWEBMAPPLUGIN_SIZE', "10,11");
	Foswiki::Func::setPreferencesValue('DIRECTEDGRAPHWEBMAPPLUGIN_EXPAND', "SEARCH,INCLUDE");
	my $output = Foswiki::Plugins::DirectedGraphWebMapPlugin::_WEBMAP(
		$this->{session},
	    new Foswiki::Attrs(""),
	    $this->{test_topic},
...
sub test_topicMapWithPrefsLinks1 {
	...
	Foswiki::Func::setPreferencesValue('DIRECTEDGRAPHWEBMAPPLUGIN_LINKS', "1");
	my $output = Foswiki::Plugins::DirectedGraphWebMapPlugin::_TOPICMAP(
		$this->{session},
	    new Foswiki::Attrs(qq()),
	    "TopicTwo",
	    $this->{test_web}
	);
...
sub test_topicMapWithLinks1OverridesPrefs {
	...
	Foswiki::Func::setPreferencesValue('DIRECTEDGRAPHWEBMAPPLUGIN_LINKS', "0");
	my $output = Foswiki::Plugins::DirectedGraphWebMapPlugin::_TOPICMAP(
		$this->{session},
	    new Foswiki::Attrs(qq(links="1")),
	    "TopicTwo",
...
sub test_topicMapWithPrefsBackLinks3 {
	...
	Foswiki::Func::setPreferencesValue('DIRECTEDGRAPHWEBMAPPLUGIN_BACKLINKS', "3");
	my $output = Foswiki::Plugins::DirectedGraphWebMapPlugin::_TOPICMAP(
		$this->{session},
	    new Foswiki::Attrs(qq()),
	    "TopicTwo",
...
sub test_topicMapWithBackLinks3OverridesPrefs1 {
	...
	Foswiki::Func::setPreferencesValue('DIRECTEDGRAPHWEBMAPPLUGIN_BACKLINKS', "0");
	my $output = Foswiki::Plugins::DirectedGraphWebMapPlugin::_TOPICMAP(
		$this->{session},
	    new Foswiki::Attrs(qq(backlinks="3")),
	    "TopicTwo",
